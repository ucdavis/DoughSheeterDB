...
static void CCONV
onPositionChangeHandler(PhidgetEncoderHandle ch, void *ctx, int positionChange, double timeChange, int indexTriggered) {

	printf("Encoder Changed: %7d  %7.3lf  %d\n", positionChange, timeChange, indexTriggered);
}
...
main(int argc, char **argv) {
	...
	*/
	...
	res = PhidgetEncoder_setOnPositionChangeHandler(ch, onPositionChangeHandler, NULL);
	if (res != EPHIDGET_OK) {
		Phidget_getErrorDescription(res, &errs);
	...
	*/
	...
	if (deviceID == PHIDID_1047) {
		printf("setting enabled\n");
		res = PhidgetEncoder_setEnabled(ch, 1);
		if (res != EPHIDGET_OK) {
