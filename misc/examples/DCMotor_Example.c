...
static void CCONV
onVelocityChangeHandler(PhidgetDCMotorHandle ch, void *ctx, double ratio) {

	printf("Velocity Changed: %.3g\n", ratio);
}

static void CCONV
onBackEMFChangeHandler(PhidgetDCMotorHandle ch, void *ctx, double backEMF) {

	printf("Back EMF Changed: %.3g\n", backEMF);
}
...
main(int argc, char **argv) {
	...
	*/
	...
	res = PhidgetDCMotor_create(&ch);
	if (res != EPHIDGET_OK) {
		fprintf(stderr, "failed to create dc motor channel\n");
		exit(1);
	}
	...
	res = PhidgetDCMotor_setOnVelocityUpdateHandler(ch, onVelocityChangeHandler, NULL);
	if (res != EPHIDGET_OK) {
		Phidget_getErrorDescription(res, &errs);
		fprintf(stderr, "failed to set velocity update handler: %s\n", errs);
		goto done;
	}

	res = PhidgetDCMotor_setOnBackEMFChangeHandler(ch, onBackEMFChangeHandler, NULL);
	if (res != EPHIDGET_OK) {
		Phidget_getErrorDescription(res, &errs);
		fprintf(stderr, "failed to set back EMF change handler: %s\n", errs);
		goto done;
	}
	...
	*/
	...
	printf("Setting Velocity to 1 for 5 seconds...\n");
	PhidgetDCMotor_setTargetVelocity(ch, 1);
	ssleep(5);
	...
	printf("Setting Velocity to 0 for 5 seconds...\n");
	PhidgetDCMotor_setTargetVelocity(ch, 0);
	ssleep(5);
