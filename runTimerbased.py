...
# the purpose of this code is to run the sheeter empty 
# the parameter for pass number, speed, and gap is from parameter.csv, which reads from the coversheet of this experiment workbook
...
def runExperiment(listOfParam):
    ...
    ####################
    # load cell only need to be started for the first pass
    if passNumber =='1':
